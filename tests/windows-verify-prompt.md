# Windows 검증 지시문 (Windows)

이 파일은 실제 Windows PC에서 터미널 코딩 에이전트를 열고 그대로 시키는 지시문이다. 저장소를 클론하면
이 파일이 함께 오므로, 에이전트에게 "tests/windows-verify-prompt.md 대로 진행해"라고 하면 된다.
...
스크립트 실행과 캡처와 문서 반영과 PR은 에이전트가 맡는다.

## 사전 준비 (한 번)

사람이 미리 갖춰 둔다.

- 터미널 코딩 에이전트, 그리고 `git`과 GitHub CLI(`gh auth login` 완료)
...
| 일 | 담당 |
|----|------|
| `windows-verify.ps1 -Smoke` 실행, 로그 해석 | 에이전트 |
| 화면 캡처(`-Capture`), 크롭, `docs/images` 반영, PR | 에이전트 |
...
에이전트가 실행한다. 명명 파이프 마운트, 파일 공유 경로, NOTICE 생성, UI 컨테이너 응답을
비대화형으로 확인한다.
...
맨 앞으로 띄운 상태에서 에이전트가 캡처한다. 캡처 명령은 카운트다운 뒤 맨 앞 창을 저장한다.
...
에이전트가 `docs/quickstart-no-cli.md`에서 아래 주석을 찾아 해당 이미지 태그로 바꾼다. 주석 문구로
찾으면 줄 번호가 바뀌어도 안전하다.
...
`.md`를 고칠 때 `docs/korean-style-guide.md`의 문체 기준을 지킨다(산문 화살표·과한 볼드 금지).
...
에이전트가 진행한다. `main` 직접 push 금지, feature 브랜치와 `gh pr create`로 올린다.

```powershell
git checkout -b docs/windows-screenshots
git add docs/images/*.png docs/quickstart-no-cli.md
