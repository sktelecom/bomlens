...
#   - the directory-structure block of docs/examples-guide.md  (catches B-6: a
...
GUIDE="$ROOT_DIR/docs/examples-guide.md"
...
for d in "$ROOT_DIR"/examples/*/; do
    ...
    if grep -Eq "(├──|└──) ${lang}/" "$GUIDE"; then
        echo "  ok:   ${lang} listed in docs/examples-guide.md"
    else
        echo "  MISS: ${lang} not in docs/examples-guide.md directory list"
        MISSING=$((MISSING + 1))
    fi
