...
- The Korean risk report prints the license classification names in English too, matching the web UI and the English report. Four of the five names were already hardcoded English while `Uncategorized` alone went through a translation key, so a Korean table read `... | Permissive | 미분류 |` — the key is gone rather than retranslated, so the five cannot drift apart again.
...
- A component linked into someone else's binary is now identified from the symbols that binary exports. A Zyxel switch declares ncurses, readline and quagga in its own notice and carries all three inside one 400 KB executable, with no version string left for any of them, so neither a signature checker nor a version-string pass could see them — they were the components a fingerprint database was going to be needed for. That binary exports 1259 dynamic symbols, among them 46 ncurses internals, 145 readline internals and 18 vtysh entry points, and a hand-written table reads them. The table matches on internals rather than public API, because libedit exports readline's entry points and `tgetent`/`tputs` belong to every termcap there has ever been, and it requires several to agree so a vendor-patched build still reads. Each judgement records the symbols it matched and the file they were in. Against that vendor's notice the image goes from 13 of 17 components to 16; the one still missing is a bootloader that is not in the download.
- Which C library a firmware carries is now named. `libc` says something is the C library, not which one, so the component went unreported — yet across the test corpus that slot is filled by three different projects with three different licences (musl is MIT, glibc and uClibc are LGPL), and a notice that says `libc`, or says nothing, has not answered the question a notice exists to answer. The library settles it inside its own file: musl writes `musl libc` into its loader banner, glibc writes `GNU C Library`. Only a SONAME listed as ambiguous has its file read this way, exactly one candidate may match, and a slot that matches none or two is reported as undecided rather than guessed.
- A version stamped in netkit's own RCS keyword is read. A NETGEAR router carries `usr/sbin/ftp` with `$NetKit: netkit-ftp-0.18-pre1 $` inside, which no checker looks for. The whole keyword is matched rather than the bare package name, and the prerelease suffix is kept — 0.18-pre1 precedes 0.18, and reporting it as the release would line the component up against advisories written for one.
- FRRouting is told apart from quagga. FRR forked from quagga in 2017 and kept its symbol names, its command library and much of its wording — an FRR `zebra` still writes `/var/tmp/quagga` — so the symbol table names the family and cannot name the member. A Zyxel XGS1210-12 carries both and declares both: its `cli` has the quagga markers and no FRR marker, its `zebra` has the reverse. The member now comes from the file's own wording, and a file that matches both markers or neither keeps the family name rather than picking one.
...
- A scan that reads someone else's SBOM now describes that document, not only the conversion of it. Every other screen reports on the CycloneDX BOM the input was converted into, which drops the facts a supplier SBOM is judged on: the format and spec version it was written in, its document name and identifier, when it was created, the tool that produced it, and the supplier or authors it names. The new Submitted SBOM section reports them, read from the original before conversion, for CycloneDX and SPDX 2.x/3.0 JSON as well as CycloneDX XML. Fields the supplier left empty are omitted rather than rendered blank, and an input whose format cannot be established produces no summary at all — on a compliance screen a confident-looking wrong value costs more than a missing one.

- Components that are known-malicious packages are now flagged, separately from the vulnerability table. A vulnerability says an honest package has a flaw you can patch; a malicious package was published to attack whoever installs it — a typosquat, a release from a hijacked maintainer account, a payload that runs at install time — and the response is removal plus credential rotation, not an upgrade, so it is reported as its own signal. The data is a bundled snapshot of the malicious-package advisories OSV publishes, reduced at image-build time from ~275 MB of archives to a 12 MB index of 232,687 packages, so the check runs fully offline like the end-of-life flag (`ENRICH_MALICIOUS=false` to skip). Matching is by PURL and never by name, since these packages are deliberately named to resemble real ones. Most advisories name no versions, meaning every published version is malicious; where an advisory does name versions, only those are flagged. Each flagged component records the advisory id and the snapshot date, because a clean result means "not in this snapshot", not "safe today".
- The web UI carries the same signal. The Overview's needs-attention list leads with malicious packages — above a failed conformance and above critical vulnerabilities, since a package published to attack the build is already running rather than waiting to be patched — and the components table badges each one, with the advisory id and snapshot date in its tooltip. The count is kept out of the severity figures on purpose: the action is removal and credential rotation, not an upgrade.

- The web UI accepts a build artifact where previously it only accepted source. A supplier who cannot hand over a source tree often can hand over what they ship, and an executable jar carries its bundled dependencies: scanning one produces 62 components and 65 vulnerabilities on the same project whose source scan finds 64, so the artifact alone is enough to assess a delivery. Accepted formats are `.jar`, `.war`, `.ear`, `.deb`, `.rpm` and `.whl`, each routed by what actually reads it — java archives and OS packages are scanned as a file, while a wheel carries no manifest readable that way and is unpacked and scanned as a directory. The list is measured rather than aspirational: formats that returned nothing either way, such as ruby gems, are not accepted at all, because receiving one would produce an empty scan that reads as a broken feature. The picker states the one real limitation, that a plain library jar has no bundled dependencies to find.
...
- The web UI carries the same check: New scan takes the outbound licence as an optional field, and the Licenses section lists the dependencies that clash with it, worst first, each with the reasoning for its verdict. When no outbound licence was declared the section says the check did not run and how to turn it on, so an empty list is never mistaken for an all-clear.

### Fixed

- Firmware that nests its own bundled sub-packages is read from the right place. The rootfs was taken from whichever `etc` directory the filesystem happened to list first, which is not depth order and not stable between machines. A MikroTik RouterOS image carries four bundled sub-packages that each look exactly like a rootfs — `bin`, `etc` and `lib`, the same entries the real root has — and one of them was picked ahead of the root, so the scan read 2 ELF files out of the image's 423 and reported no components at all for an image that unpacked 587 files. The shallowest candidate now wins, with ties broken on the path, and that image goes from 0 to 309 components. The other six vendor images are byte-identical.
- A Linux kernel module no longer carries a CPE built out of its own name. syft gives each `.ko` file an identifier made from the module name — `8021q.ko` becomes `cpe:2.3:a:8021q:8021q:1.8`, where the 1.8 is the module's `modinfo` field rather than a release of anything — and most of those invented products match nothing, which is why it had cost nothing so far. The collisions are real though: a MikroTik image carries a `wireguard` module at 1.0.0 and `wireguard:wireguard` is in the bundled index at 0.5.3, so only the version kept them apart. The module stays in the SBOM with its licence; what goes is the guess about its identity. Three images in the test corpus are affected, 304 modules on the largest.
- The NOTICE no longer lists a firmware's kernel modules one by one. Each arrived as three lines whose source read `pkg:generic/nls_utf8`, a purl built from the module name that leads nowhere, and the obligation is discharged by the kernel they were built from, which the notice already lists in its own right. They fold into one line per licence naming their count and their kernel. Measured on a MikroTik RouterOS image: 1689 lines to 785.
- The NOTICE no longer lists the files a scan walked as though they were components. A scan records them as CycloneDX `type: file` entries, which is a legitimate inventory, but a path is not a project — there is no name to attribute and no source to point at. Across the test corpus all 4,649 such entries carried no licence, no purl and no external reference, so every one landed under NOASSERTION with an empty attribution beneath it. They fold the same way, with the count kept in the licence header. This reaches beyond firmware: a default scan records the artifact it was given the same way, so a jar scan's notice was listing its own container mount path. Measured: a Ubiquiti UniFi notice goes from 1744 lines to 900.

- A source scan no longer modifies the project it scans. Dependency resolution has to run in the mounted tree, because the build tools need the real sources, so `go mod tidy` rewrote `go.mod` and created `go.sum`, `cargo generate-lockfile` wrote `Cargo.lock`, `bundle lock` wrote `Gemfile.lock`, `swift package resolve` wrote `Package.resolved`, and gradle and maven left their build directories behind: scanning a checkout left the working tree dirty, and in CI the scan produced the very diff it then reported on. The resolver-owned files are now snapshotted before the run and put back afterwards, and files or build directories that were not there before are removed. Nothing outside those names is considered and nothing that already existed is deleted, so a committed lockfile or a pre-existing `build/` is never lost, and the restore also runs when a scan is aborted. Set `BOMLENS_KEEP_BUILD_OUTPUT=1` to keep the resolved tree for inspection.
- The generated SBOM is no longer written into the scanned tree first. cdxgen now writes it straight to the run's working directory when that directory is on a mount the scanner container shares; where it is not, the previous write-then-move behaviour still applies.
