...
- 권장 항목이 미달이면 `warn`이며, `fail`로 보지는 않습니다. 라이선스와 hash 커버리지 외에, 규제 기준선이 요구하는 컴포넌트별 권고 필드도 여기에 포함됩니다 — SHA-512 체크섬 커버리지, 컴포넌트 작성 주체, 컴포넌트 파일명, 소스·배포 URI, 전달 파일 속성(스캔으로 확인할 수 없으면 검토 필요로 표시).
- HTML 보고서 상단 카드에 적합/부적합과 누락 목록이 표시됩니다.
- 규제 기준선과 대응되는 검사에는 행 아래에 참고 규정이 붙고, 크로스워크 절이 프레임워크별 충족 현황을 집계합니다 — BSI TR-03183-2(EU 사이버복원력법을 위한 독일 기술지침)와 미국 NTIA 최소 요소입니다. 크로스워크는 참고 자료이며 준수 판정을 하지 않습니다. 동작 방식은 [AI 모델 SBOM 가이드](ai-model.ko.md#규제-크로스워크)에서 설명합니다.
...
Yocto 빌드는 자체 SPDX SBOM을 만들 수 있고, 그 파일을 그대로 올리면 됩니다. 일반 SPDX 경로가 아니라 전용 파서가 읽습니다. Yocto 문서에는 일반 경로가 잃어버리는 정보가 두 가지 들어 있기 때문입니다.
...
산출물은 `tmp/deploy/images/<machine>/<image>.rootfs.spdx.json` 경로에 생깁니다. 이 파일을 올리면 됩니다.
...
SPDX 3.0만 지원합니다. Yocto 4.0 Kirkstone과 5.0 Scarthgap은 기본값이 SPDX 2.2인데, 이 형식은 파일 하나가 아닙니다. 최상위 `.spdx.json`은 색인이고 실제 패키지는 옆에 있는 `.spdx.tar.zst` 안의 레시피별 문서에 들어 있습니다. 색인만 올리면 빈 결과를 조용히 내놓지 않고 그 사실을 알려 줍니다. 두 릴리스 모두 위 설정으로 SPDX 3.0을 만들 수 있습니다.

PURL에 기대는 적합성 항목은 실패합니다. Yocto는 패키지를 PURL이 아니라 CPE로 식별하므로 PURL 적용률과 거기서 파생되는 항목은 통과할 수 없습니다. SBOM의 결함이 아니라 식별자 표기 방식의 차이입니다.

업로드 크기는 100MB까지 받습니다. 참고로 기준 이미지인 `core-image-minimal` 문서는 설치 패키지 35개에 15.8MB입니다.
