...
자체 생성 스캔도 이제 기본으로 같은 적합성 보고서 파일을 만듭니다([입력 시나리오별 처리](by-input.ko.md#4종-산출물) 참고). 어느 쪽이든 그 요약은 위험분석보고서 1절에 들어갑니다. 다만 아래에서 설명하는 판정 화면은 `--analyze` 경로에서만 나타납니다. 자기가 만든 SBOM이 자기를 채점하는 건 대부분의 검사 항목에서 의미 있는 신호가 아니기 때문입니다. 자체 생성한 SBOM을 화면으로 보려면 그 결과물을 다시 `--analyze`로 넣으세요.
