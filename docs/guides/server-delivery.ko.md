...
이 가이드는 서버를 세 층으로 보고 각 층을 BomLens로 스캔합니다. 세 SBOM을 따로 제출하는 것이 기본이며, 그래야 각 층을 그 자체로 검토할 수 있습니다. 제출 시스템이 단일 파일을 요구할 때만 하나의 제품 SBOM으로 병합합니다([선택: 단일 SBOM으로 병합](#선택-단일-sbom으로-병합) 참고).
...
> **Windows**: 아래 명령은 macOS/Linux 기준입니다. `scan-sbom.bat`와 WSL2 사용법은 [시작 가이드](../start/first-scan.md#installation)를 참고하세요.
...
세 SBOM을 그대로 제출합니다. 합친 파일이 아니라 각 SBOM을 따로 확인해, 문제를 그 층에서 바로 잡습니다. 각 SBOM이 올바른 형식이고 컴포넌트가 실제 purl을 갖는지 봅니다.
