...
> The open-source risk report (`_risk-report`) is generated by default in every analysis mode (license + vulnerability tally, with response deadlines). To skip it, use `--no-report`. For handling each of the five input forms, see the [Scenarios guide](by-input.md).
