...
매치는 출처와 신뢰도가 태깅된 채 읽기 전용으로 기록됩니다. BomLens는 accept/reject 같은 audit 워크플로를 제공하지 않습니다. 매치를 확정하거나 triage해야 하면 SBOM을 TRUSCA에 올려 거기서 처리하세요.

## 준비

SCANOSS 클라이언트를 포함한 이미지를 빌드(또는 pull)합니다.
