...
2. `syft`로 패키지 매니저(opkg, dpkg, apk, rpm)가 설치한 구성요소를 식별합니다. 하나의 이미지가 파일시스템을 여럿 담을 수 있습니다. 컨테이너 이미지 저장소가 rootfs 안이 아니라 옆에 놓인 경우(스위치 운영체제에서 흔한 구조로, 실제로 동작하는 것 대부분이 컨테이너 안에 있습니다) 그 계층까지 함께 식별합니다. rootfs만 읽으려면 `FW_EXTRA_ROOTS=false`로 둡니다.
