...
**모델·데이터셋** — 각 모델 카드의 식별자, 아키텍처, 태스크, 라이선스, 공급자, 무결성과 공개 4축 패널(가중치 / 아키텍처 / 학습 데이터 / 학습 과정 — 모델 카드에 문서화된 범위), 그리고 모델이 참조하는 데이터셋을 라이선스·콘텐츠 해시·파생 출처와 함께 표로 보여 줍니다.
...
- 검토 필요(10건) — 자동 출처가 없어 사람이 직접 확인해야 합니다. 같은 결과가 세 가지 형식으로 나옵니다. 기계용 `{Project}_{Version}_conformance.json`(CI 게이트, 비교용), 표 형태의 `_conformance.md`, 시각화한 `_conformance.html`입니다. AI SBOM에서는 각 형식에 [규제 크로스워크](#규제-크로스워크)도 함께 담기며, JSON에서는 매핑된 요소가 하나 이상 검사됐을 때만 `regulatoryCrosswalk` 객체로 나타납니다.
