...
| Keras(`.h5`/`.keras`) | 형식 자체와 `Lambda` 레이어 존재 여부 |
| npz, npy | 내부 항목 목록 |
...
pickle은 데이터 형식이 아닙니다. 불러오는 순간 스트림이 지시하는 코드를 그대로 실행하므로, 이 형식의 체크포인트는 그 자체가 위험 요소입니다. 라이선스와도, CVE와도 별개입니다. Keras `.h5`/`.keras` 파일도 형태만 다를 뿐 같은 종류의 위험을 가집니다. `Lambda` 레이어에 `marshal`로 직렬화한 파이썬 함수를 담을 수 있는데, 이 함수는 Keras가 레이어를 복원할 때 실행됩니다. HuggingFace에 있는 모델은 Hub가 돌린 검사 결과를 읽어 옵니다. 공개하지 않은 파일은 그런 기록이 없으므로 여기서 직접 검사합니다. pickle 계열 형식은 Hub와 같은 도구인 [picklescan](https://github.com/mmaitre314/picklescan)을, Keras는 `Lambda` 표식을 찾는 단순 텍스트 검사를 씁니다. 복원할 필요 없이 찾을 수 있는 표식이기 때문입니다.

판정은 모델 컴포넌트에 기록되고 위험 판정의 파일 보안 축에 반영됩니다.

| 판정 | 뜻 | 위험 등급 |
| --- | --- | --- |
| 코드를 실행하는 항목 발견 | pickle 스트림에서 `os.system` 같은 위험한 항목에 닿을 수 있음 | caution |
| 확인이 필요한 항목 있음 | 코드가 될 수 있는 pickle 항목(대개 사용자 정의 클래스), 또는 `Lambda` 레이어가 있는 Keras 파일 | review |
| 코드를 실행하는 위험 없음 | pickle 데이터에 그런 항목이 없거나, Keras 파일에 `Lambda` 레이어가 없음 | ok |
| 코드를 실행하지 않는 형식 | GGUF, safetensors, ONNX, 일반 npy | ok |
| 읽지 못함 | 파일이 해석되지 않음 | review |

가운데 두 줄은 짚어 둘 것이 있습니다. "확인이 필요한 항목"은 멀쩡한 체크포인트에서도 흔합니다. 모델이 자체 클래스를 담고 있거나, 평범한 활성화 함수를 위해 `Lambda` 레이어를 썼어도 걸립니다. 그래서 경보를 울리지 않고 사람이 보도록 남깁니다. 그리고 깨끗하다는 결과는 질문 하나에만 답한 것입니다. 이 경로로는 불러올 때 코드를 실행하지 않는다는 뜻이지 파일에 아무 문제가 없다는 뜻이 아닙니다. pickle은 pickle 분석이지 악성코드 검사가 아니고, Keras는 `Lambda` 표식을 찾는 텍스트 검색이지 모델 구성을 해석한 것이 아닙니다.
...
연구 데이터는 논문이 올려 둔 곳에 있고, 여러 분야에서 그곳은 모델 허브가 아니라 Figshare 같은 저장소입니다. HuggingFace 모델 id 대신 `--model`에 해당 항목을 그대로 넣습니다.
...
페이지 주소, DOI, 항목 번호 모두 됩니다. 최신 버전이 아니라 특정 버전을 읽으려면 버전이 표시된 주소나 DOI를 주면 됩니다(`.../33412285/2`, `10.6084/m9.figshare.33413521.v1`). 계정도 별도 이미지도 필요 없습니다. 공개 항목 조회는 인증 없이 응답하고, 변환은 기본 이미지에 들어 있습니다.

항목은 CycloneDX `data` 컴포넌트가 되며 라이선스, DOI, 저자, 파일별 MD5 값을 담습니다. 표준 식별자로 옮길 수 있는 라이선스(Creative Commons 계열, MIT, Apache 2.0, GPL 계열, CC0)는 SPDX 식별자로 기록하고, 그 밖의 것은 추측하지 않고 표기 그대로 남깁니다. `--usage`는 모델과 마찬가지로 적용됩니다. 비상업 조건은 내부 연구용일 때와 외부에 제공할 때 의미가 다릅니다.

이름에 "figshare"가 없는 기관 DOI(`10.25916/sut.33412285.v1`)는 다른 DOI와 구분할 수 없으므로 항목 주소를 주십시오. 비공개, 엠바고, 철회된 항목은 계정 없이 읽을 수 없으며, 라이선스가 없는 데이터셋으로 기록하지 않고 오류로 알립니다.
...
![적합성 — AI SBOM의 G7 권고 하위 블록](../images/web-ui-g7-ko.png)
...
- 검토 필요(10건) — 자동 출처가 없어 사람이 직접 확인해야 합니다.

같은 결과가 세 가지 형식으로 나옵니다. 기계용 `{Project}_{Version}_conformance.json`(CI 게이트, 비교용), 표 형태의 `_conformance.md`, 시각화한 `_conformance.html`입니다. 각 형식에는 [규제 크로스워크](#규제-크로스워크)도 함께 담기며, JSON에서는 매핑된 검사가 하나 이상 평가됐을 때만 `regulatoryCrosswalk` 객체로 나타납니다.
...
이 스캔이 낸 보고서를 그대로 열어 볼 수 있습니다. 베이스 모델의 [적합성 보고서](../samples/aether-7b-5attn_conformance.ko.html)이며, AI 준수 개요가 담던 클러스터별 충족 현황과 검토 대상 라이선스가 이 보고서 맨 위에 함께 나옵니다. 한국어 보고서는 `--lang ko`로 생성합니다.
...
그래도 규제와 무관하지 않습니다. EU 인공지능법(AI Act)의 고위험·투명성 의무가 2026년 8월 2일부터 적용되고, Annex IV가 요구하는 기술 문서는 G7 클러스터와 상당 부분 겹칩니다. 적합성 보고서가 주는 것은 가시성입니다. 모델 문서가 이미 다루는 요소와 사람이 채워야 할 요소를 항목별로 보여주므로, 준수 판정이 아니라 준비를 돕는 구체적인 방법이 됩니다(이 보고서가 인증하지 않는 것은 [한계](#한계)를 참고하세요).
...
위 크로스워크는 SBOM 데이터 필드만 다룹니다. 이 필드가 속한 CRA 준비 절차 전체(거버넌스, 취약점 대응, 기술문서, 보안 업데이트)는 커뮤니티가 관리하는 [OpenChain CRA Compliance Requirements & Checklist](https://github.com/OpenChain-Project/CRA-Compliance)를 참고하세요.
...
크로스워크는 준수 판정이 아니라 준비를 돕는 자료입니다([한계](#한계) 참고). 각 매핑에는 그 연결의 해석 근거를 함께 담아 사람이 판단할 수 있게 했고, 크로스워크가 검사 상태나 전체 판정을 바꾸지는 않습니다. 검사가 이미 낸 결과를 규제별로 다시 묶을 뿐입니다.
...
- 적합성 보고서는 EU 인공지능법, 한국 AI 기본법을 비롯한 어떤 규제의 준수도 인증하지 않습니다. 문서화 공백을 드러내 사람이 메울 수 있게 할 뿐입니다.
