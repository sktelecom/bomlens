...
설치하기 전에 결과가 어떤 모습인지 보고 싶다면 [데모](https://sktelecom.github.io/bomlens/demo/)를 둘러보세요. 입력 종류별로 완료된 스캔을 하나씩 담은 실제 웹 UI입니다. 소스로 스캔한 Spring Boot 프로젝트(컴포넌트 71개, 알려진 취약점 65건), 컨테이너 이미지, CycloneDX ML-BOM으로 만든 AI 모델, 그리고 형식 요건을 검사한 공급사 SBOM입니다. 설치할 것이 없고 아무것도 업로드되지 않습니다. 평범한 로컬 실행으로 나온 결과를 그대로 담아 둔 화면입니다.
