...
> [비개발자 빠른 시작](../start/no-cli.ko.md)부터 보세요.
...
- CLI 래퍼(`scripts\scan-sbom.bat`)를 쓴다면 [Git for Windows](https://git-scm.com/download/win)(Git Bash)가 필요합니다.
- WSL2 경로를 쓴다면 둘 다 필요 없습니다. WSL 안에서 `.sh`를 직접 실행합니다.

## 설치

설치 방법은 운영체제에 따라 다릅니다. Windows에서 CLI 없이 쓰려면 아래 다운로드 방식이 가장 쉽고, macOS와 Linux, WSL2에서는 CLI로 설치합니다.

### Windows — 다운로드 후 더블클릭 (CLI 불필요)

가장 간단한 길은 데스크톱 앱입니다. [Windows용 BomLens 내려받기 (.exe)](https://github.com/sktelecom/sbom-tools/releases/latest/download/SBOM-Generator-Setup.exe)를 눌러 받은 파일을 더블클릭하면 콘솔 창 없이 UI가 열립니다. 아직 미서명이라 SmartScreen 경고가 뜨면 "추가 정보"를 누르고 "실행"을 고릅니다. 단계별 안내는 [비개발자 빠른 시작](../start/no-cli.ko.md)을 참고하세요.
...
CLI를 쓰고 싶다면 [Git for Windows](https://git-scm.com/download/win)(Git Bash)를 설치한 뒤 `scripts\scan-sbom.bat`를 사용하거나, WSL2에서 아래 macOS/Linux 절차를 그대로 따르면 됩니다.
...
이미지 크기는 약 3–4 GB입니다. 네트워크 상황에 따라 몇 분이 걸릴 수 있습니다.
...
```
Windows에서는(Rancher Desktop, Docker Desktop) `scripts\sbom-ui.bat`를 더블클릭합니다. UI를 실행한 폴더가 산출물 저장 위치가 됩니다. 이 폴더는 Docker 엔진의 파일 공유에 포함된 경로여야 하는데, 홈 디렉터리(`C:\Users\...`) 아래는 두 엔진 모두 기본으로 공유되므로 안전합니다. 공유되지 않은 위치에서 실행하면 스캔이 끝나도 산출물이 PC에 나타나지 않습니다. WSL2 + docker-ce를 쓴다면 `.bat` 대신 WSL 안에서 `scan-sbom.sh --ui`를 실행하며, 이 파일 공유 제약도 없습니다. 포트가 충돌하면 `UI_PORT=9090`을 앞에 붙입니다.
...
UI 화면 구성과 스캔 대상별 상세는 [고지문·보안 보고서 가이드](../reference/ui.ko.md)를 참고하세요. 아래는 같은 작업을 CLI로 하는 방법입니다.
...
ZIP 소스(`--target app.zip`), 기존 SBOM(`--analyze sbom.json`), 펌웨어(`--target dev.bin --firmware`) 등 입력 형태별 처리는 [시나리오별 가이드](../guides/by-input.ko.md)에 정리되어 있습니다.
...
> `--generate-only` 옵션은 산출물을 업로드 없이 로컬에만 저장합니다(취약점 스캔은 그대로 수행). `--all`을 함께 쓰면 고지문, SBOM, 오픈소스 위험분석 보고서가 한 번에 생성됩니다. 전체 옵션은 [사용 가이드](../reference/cli.ko.md#옵션-레퍼런스)를 참고하세요. 결과를 TRUSCA나 Dependency-Track에 올리려면 `--trusca <project_id>`(또는 `UPLOAD_TARGET`)를 쓰며, 절차는 [사용 가이드의 업로드 섹션](../guides/upload.ko.md)에 있습니다.
...
아래 예시는 `jq`를 사용합니다. WSL2(우분투)에는 `sudo apt-get install jq`로, Windows Git Bash에는 기본 포함되지 않으니 [jq 릴리스](https://jqlang.github.io/jq/download/)에서 받거나 `winget install jqlang.jq`로 설치하세요. 설치가 번거롭다면 웹 UI의 개요 화면에서 컴포넌트 수와 라이선스를 바로 확인할 수 있습니다.
