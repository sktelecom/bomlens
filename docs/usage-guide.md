...
| 환경변수 | 기본값 | 설명 |
|----------|--------|------|
| `SBOM_SCANNER_IMAGE` | `ghcr.io/sktelecom/sbom-scanner:latest` | 스캐너 이미지를 다른 태그로 재정의 (`bomlens`와 같은 이미지) |
...
분석 대상의 유형에 따라 내부적으로 적합한 도구(cdxgen 또는 syft)가 자동으로 선택됩니다. 선택 로직의 자세한 내부 동작은 [아키텍처](architecture.md#입력-타입별-분기) 문서에 있습니다.

### 소스 코드 분석 (cdxgen)

패키지 매니저 파일(`pom.xml`, `package.json`, `go.mod` 등)을 파싱해 의존성 목록을 추출합니다.
...
- 취약점 대응 기한은 SKT 검증 프로세스를 따라 Critical은 7일, High는 30일 이내입니다.

### Docker 이미지 분석 (syft)

설치된 OS 패키지와 애플리케이션 패키지를 분석합니다.
