...
> **환경변수**: `SBOM_SCANNER_IMAGE`(스캐너 이미지 재정의), `SBOM_FIRMWARE_IMAGE`(펌웨어 이미지), `GIT_TOKEN`(비공개 git 클론), `COSIGN_KEY`(서명 키), `FETCH_LICENSE`(기본 true, 소스 스캔 시 의존성 라이선스 자동 조회. `false`면 조회를 생략해 속도를 높임), `SECURITY_ENRICH`(기본 true, 보안 보고서에 EPSS와 CISA KEV 신호 보강. 폐쇄망에서는 `false`로 두면 외부 조회를 생략). 출력 플래그 상세는 [고지문·보안·UI 가이드](notice-security-ui-guide.md)를, 공급사 SBOM 분석은 [공급사 SBOM 분석](supplier-sbom-analysis.md)을 참고하세요.
...
```bash
SBOM_SCANNER_IMAGE="ghcr.io/sktelecom/sbom-generator:1.2.0" \
  ./scripts/scan-sbom.sh --project "MyApp" --version "1.0.0" --generate-only
```
...
2. Docker 이미지를 최신 버전으로 업데이트합니다: `docker pull ghcr.io/sktelecom/sbom-generator:latest`
