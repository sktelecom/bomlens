...
`_risk-report.{md,html}`(오픈소스위험분석보고서)는 모든 분석 모드(소스·아카이브·GitHub·이미지·바이너리·RootFS·펌웨어·SBOM 분석)에서 기본으로 생성됩니다. 라이선스(고지문)와 취약점(보안) 데이터를 집계하므로 고지문·보안 스캔이 자동으로 함께 켜집니다.
