...
| # | 질문 | 결론 |
|---|------|------|
| 1 | Docker 이미지의 의미 / plain cdxgen 대비 | **주류 언어(java·python·node·dotnet·php)는 cdxgen 공식 이미지로 검출 동일. go·ruby·rust만 우리 이미지가 우수(cdxgen 공식에 cargo·bundler 등 부재). 통합 기능(syft·고지문·보안·UI)이 핵심 차별점.** `compare-cdxgen-vs-docker.sh`로 재현(빌드 안 된 순수 소스 기준). |
...
핵심 질문은 "cdxgen이 내부 install로 충분하다면 우리 이미지가 필요한가"이다. cdxgen은 lockfile이 없어도 **스스로 패키지 매니저를 호출해 transitive까지 해석**한다(우리 예제는 lockfile 미커밋인데도 검출됨). 단, **그 install이 작동하려면 해당 언어의 도구가 환경에 있어야 한다.**

### 실측 데이터 (cdxgen 공식 이미지 vs 우리 이미지, `compare-cdxgen-vs-docker.sh`)

"우리 이미지 없이 plain cdxgen을 쓸 때"(= cdxgen 공식 이미지, 내부 install 켜짐, 자체 toolchain 포함)와 우리 이미지를 번들 예제(lockfile 미커밋)로 비교:
...
→ **주류 5개 언어(java·python·node·dotnet·php)는 cdxgen 공식 이미지로 충분**(검출 동일). **go·ruby·rust에서만 우리 이미지가 결정적으로 우수** — cdxgen 공식 이미지에 cargo·bundler·완전한 go 도구가 없기 때문(ruby는 0, rust는 36배).
...
### 권고 + 다음 단계
- **순수 SBOM 검출**만 보면 주류 언어는 cdxgen 공식 이미지로 충분하다. 우리 이미지의 차별적 가치는: ① **go·ruby·rust 등 cdxgen 공식이 약한 언어 커버리지**(한 이미지로 전 언어) ② cdxgen이 **하지 않는** 것 — syft 이미지/바이너리/RootFS 스캔, 고지문, Trivy 보안보고서, 웹 UI 통합 ③ 도구 버전 고정·결정론적 출력.
- 시사점: 만약 대상이 주류 언어 위주라면, 무거운 통합 이미지 대신 cdxgen 공식 이미지 + 후처리(고지문/보안)만으로도 상당 부분 커버 가능 — §5 "언어별 이미지 분리" 로드맵과 연계해 이미지 경량화를 재검토할 여지가 있다.
...
5. **언어별 이미지 분리 + on-demand pull** — `scan-sbom.sh`가 프로젝트 타입을 감지해 필요한 cdxgen 공식 언어 이미지만 pull. 거대 단일 이미지의 pull 비용을 줄인다. (**구현 완료** — `detect_lang()`/`img_for_lang()`, 후처리는 별도 경량 이미지로 분리. → §5 Phase 5)
...
#### Phase 5 — 고도화 (로드맵)
- 언어별 이미지 분리 + on-demand pull (**구현 완료** — `scan-sbom.sh`의 `detect_lang()`/`img_for_lang()`가 언어를 감지해 cdxgen 공식 언어 이미지를 on-demand로 pull·실행. 아키텍처 상세는 [architecture.md](architecture.md) 참조), cosign 서명(**구현 완료**), 빌드환경 자동감지 함수화.
- **번들 도구 라이선스 컴플라이언스** — 조합 도구들의 라이선스 인벤토리·배포 의무는 [../THIRD_PARTY_LICENSES.md](../THIRD_PARTY_LICENSES.md) 참조. (문서화 완료: [../THIRD_PARTY_LICENSES.md](../THIRD_PARTY_LICENSES.md), [../NOTICE](../NOTICE))

#### Phase 6 — 펌웨어 분석 (예정)
공급사 제공 네트워크 장비 펌웨어의 SBOM/취약점/라이선스 점검. **설계·조사 문서화는 완료**([firmware-analysis.md](firmware-analysis.md))이며, **실제 코드 구현은 후속 작업**으로 진행한다.

- **상태**: 설계 확정·문서화 완료, 코드 미구현.
- **선행 조건**: 현재 진행 중인 별도 작업(타 세션)을 마무리한 뒤 착수.
- **확정 방향**: 언팩 unblob(폴백 BANG) → syft + cve-bin-tool → SBOM 병합 → 기존 후처리 재사용. 별도 opt-in 이미지 `sbom-scanner-firmware`로 격리. 목표 범위 = Phase 1+2(상세 단계는 [firmware-analysis.md §6](firmware-analysis.md)).
- **착수 시 작업 항목**: `docker/lib/scan-firmware.sh` 신규, `docker/entrypoint.sh` FIRMWARE case, `scripts/scan-sbom.sh` `is_firmware()`/`--firmware`/디스패치, `docker/Dockerfile` `SBOM_FIRMWARE` opt-in 블록 + 펌웨어 이미지 LABEL(라이선스/소스 오퍼), 기본 이미지 GPL-free CI 점검, 테스트 fixture. 함수 핑거프린팅(SCANOSS/LibDB 등)은 Phase 3 선택지로 보류.

#### Phase 7 — 공급사 제출 SBOM 검증·분석 (예정)
SKT 공급망 가이드([for-suppliers](https://sktelecom.github.io/guide/supply-chain/for-suppliers/), [requirements](https://sktelecom.github.io/guide/supply-chain/for-suppliers/requirements/))에 따라 공급사가 제출한 SBOM을 **검증 → 라이선스/취약점 분석 → 위험 보고서 → 대응 요구**한다. sbom-tools를 "생성기"에서 "공급사 SBOM 검증·분석 도구"로 확장. **설계·문서화 완료**([supplier-sbom-analysis.md](supplier-sbom-analysis.md)), **코드 구현은 후속**.

- **상태**: 설계 확정·문서화 완료, 코드 미구현.
- **확정 방향**: `--analyze <sbom>` 입력(CycloneDX/SPDX) → `validate-sbom.sh`(SKT 요구사항 충족 검증, 원본 기준) → `convert-to-cdx.sh`(SPDX→CDX, `syft convert` 재사용) → 기존 normalize/notice/security **무수정 재사용** → `generate-risk-report.sh`(Critical 7일/High 30일 대응 기한 포함). 목표 = 전체(검증+분석+보고서), 웹 UI 업로드는 후순위.
- **착수 시 작업 항목**: `docker/lib/validate-sbom.sh`·`convert-to-cdx.sh`·`generate-risk-report.sh` 신규, `docker/entrypoint.sh` `ANALYZE` case, `scripts/scan-sbom.sh` `--analyze`/마운트/디스패치, (후순위) `docker/web/server.py` 업로드. 상세는 [supplier-sbom-analysis.md §3-7](supplier-sbom-analysis.md).
- **역할 경계**: sbom-tools = 로컬 단일 SBOM 검증·분석·보고서. 전사 등록·triage·대응 추적은 TOSCA/trustedoss-portal 범위.
