...
- **구현**: 언팩 unblob(폴백 unsquashfs) → syft + cve-bin-tool → SBOM 병합 → 기존 후처리 재사용. GPL 도구는 별도 opt-in 이미지 `sbom-scanner-firmware`로 격리(기본 이미지는 GPL-free). `--firmware`(또는 펌웨어 확장자 자동감지)로 진입. 관련: `docker/lib/scan-firmware.sh`, `entrypoint.sh` FIRMWARE case, `Dockerfile` `SBOM_FIRMWARE` 블록.

#### Phase 7 — 공급사 제출 SBOM 검증·분석 (구현 완료)
SKT 공급망 가이드([for-suppliers](https://sktelecom.github.io/guide/supply-chain/for-suppliers/), [requirements](https://sktelecom.github.io/guide/supply-chain/for-suppliers/requirements/))에 따라 공급사가 제출한 SBOM을 **검증 → 라이선스/취약점 분석 → 위험 보고서**로 처리. sbom-tools를 "생성기"에서 "검증·분석 도구"로 확장. 상세는 [supplier-sbom-analysis.md](supplier-sbom-analysis.md).

- **상태**: 검증+분석+보고서+**웹 UI 업로드** 모두 구현·머지 완료.
- **구현**: `--analyze <sbom>`(CycloneDX/SPDX) → `validate-sbom.sh`(요구사항 충족 검증) → `convert-to-cdx.sh`(SPDX→CDX) → 기존 normalize/notice/security 재사용 → `generate-risk-report.sh`(Critical 7일/High 30일). 웹 UI는 "SBOM 업로드"로 동일 처리. 관련: `docker/lib/{validate-sbom,convert-to-cdx,generate-risk-report}.sh`, `entrypoint.sh` ANALYZE case, `server.py` 업로드.
- **참고**: 위 위험분석보고서(`generate-risk-report.sh`)는 ANALYZE 전용이 아니라 **모든 모드에서 기본 생성**되도록 일반화됨(`--no-report`로 opt-out).
