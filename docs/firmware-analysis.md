...
> **구현 완료(Phase 1+2).** 기존 IMAGE/BINARY/ROOTFS 모드와 동일하게 **후처리 이미지 패밀리의 한 모드**로 추가하되, 무거운 언팩·바이너리 분석 도구는 별도 opt-in 이미지 `sbom-scanner-firmware`에만 설치합니다. 언팩 폴백 순서는 **unblob → BANG(설치 시) → unsquashfs(표준 squashfs) → binwalk(정상 설치 시)**이며, 각 단계는 종료코드가 아니라 **실제 추출 파일 유무**로 판정합니다.
