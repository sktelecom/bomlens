...
출력 베이스 아래 각 스캔의 `{Project}_{Version}/` 하위 폴더가 하나의 과거 스캔이며, 레일의 최근 목록에 나타납니다(최신 20개, 최고 심각도 표시). 클릭하면 그 결과를 다시 열고, 삭제하면 그 하위 폴더가 제거됩니다. 계정이나 데이터베이스 없이 로컬 파일만 사용합니다.
