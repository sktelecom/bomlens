...
The G7 AI minimum-element checks are added only when the SBOM has a model component. For an AI SBOM generated by the scan itself they appear on **Models & datasets**, below the model cards; for one uploaded for review they appear on **SBOM Validation** instead, alongside the base format checks. Either way, the Overview leads an AI scan with a summary card — the model's risk verdict, the G7 headline and the regulatory framework coverage (EU AI Act, Korea AI Basic Act) — that links into whichever of the two sections carries the detail. It is advisory, not a certification, and the same data is written to the `_ai-profile` files (see [Artifacts](artifacts.md)).
