...
| `{Project}_{Version}_ai-profile.json` / `.md` / `.html` | AI SBOM (`--model`, 또는 모델 컴포넌트가 있는 SBOM에 `--analyze`) | AI 준수 개요: G7 요약, 메울 수 있는 공백과 참고 링크, 라이선스 표시 컴포넌트, 규제 크로스워크 (참고용, 인증 아님). 예시는 [AI 준수 개요](../samples/aether-7b-5attn_ai-profile.ko.html)를 참고한다 |
