...
표는 KEV 포함 항목을 맨 위에 두고, 그다음 심각도, 마지막으로 EPSS 내림차순으로 정렬합니다. 위에서부터 처리하면 자연히 위험이 큰 것부터 대응하게 됩니다. 이 정렬은 정적 보고서 파일에만 적용됩니다. [웹 UI의 취약점 표](../reference/ui.ko.md#결과-섹션)는 심각도로 정렬한 뒤 CVSS로 다시 정렬한 순서로 열리며, EPSS를 포함한 모든 열 기준으로 다시 정렬할 수 있습니다.
...
선언하지 않으면 검사하지 않으며, 보고서에서 해당 절이 아예 빠집니다. 아무도 확인하지 않은 결과를 문제없음으로 보여 주지 않기 위해서입니다. 법적 판단이 아니라 문서 준비를 돕는 자료입니다. 플래그는 [CLI 레퍼런스](../reference/cli.ko.md), 판정이 표시되는 위치는 [웹 UI](../reference/ui.ko.md)를 참고하세요.
