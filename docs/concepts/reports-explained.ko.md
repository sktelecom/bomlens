...
- 스캔 도구가 라이선스 이름을 정하지 못해 `CUSTOM`으로 남기고 라이선스 파일 전문만 붙여 둔 경우(저작권 표기 줄이 바뀐 LICENSE 파일을 쓰는 Go 모듈에서 흔합니다), 그 전문을 주요 라이선스(`MIT`, `BSD-2-Clause`, `BSD-3-Clause`, `Apache-2.0`, `ISC`)의 조항 문구와 대조해 SPDX 식별자를 복구합니다. 대조해도 애매한 전문은 추측하지 않고 `CUSTOM`으로 둡니다.
