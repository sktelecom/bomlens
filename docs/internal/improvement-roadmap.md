...
이 문서는 실제 스캔 결과에서 드러난 미비점을 정리하고, 오픈소스 거버넌스 포털 TRUSCA(구 TrustedOSS Portal)를 참고 기준으로 삼아 개선 항목과 우선순위를 도출한다. 웹 UI의 스캔 엔진 문제는 이미 해결했고(아래 1항), UI 가시성과 리포트 충실도는 남은 과제다.
