...
#### Phase 5 — 고도화 (로드맵)
- 언어별 이미지 분리와 on-demand pull(구현 완료. `scan-sbom.sh`의 `detect_lang()`/`img_for_lang()`가 언어를 감지해 cdxgen 공식 언어 이미지를 on-demand로 pull·실행한다. 아키텍처 상세는 [architecture.md](../concepts/architecture.ko.md) 참조), cosign 서명(구현 완료), 빌드환경 자동감지 함수화.
