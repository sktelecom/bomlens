...
- **cdxgen `aibom`** — 문서상 HuggingFace PURL·URL, Modelfile, GGUF에서 CycloneDX AI/ML BOM을 생성한다고 소개됩니다. 출력이 CycloneDX 1.7이라 AI 경로 포맷 결정과 맞고, 기존 cdxgen 통합·후처리에 붙이기 좋습니다. **다만 생성 스파이크(부록 A)에서 12.2.0과 핀 버전 12.5.0 모두 HF 모델 디렉토리·Modelfile을 0 components로 처리했고, lib에 huggingface·gguf·safetensors 수집 코드가 없었습니다.** modelCard 출력 스키마는 있으나 모델을 읽어 채우는 단계가 이 버전엔 없습니다. 1차 엔진으로 확정하기 전에 모델 ingestion이 실제로 되는 버전·호출을 검증해야 합니다.
- **OWASP AIBOM Generator** — HuggingFace 모델에서 CycloneDX를 생성하고 완전성 점수를 매깁니다. cdxgen의 ingestion 공백이 확인된 만큼, 생성 엔진 1순위 후보로 cdxgen과 함께 비교 평가합니다. 완전성 점수 개념은 G7 적합성 검사 설계에도 참고할 만합니다.
- **커스텀 추출기(대안)** — 스파이크에서 HF 모델 디렉토리의 `README.md` frontmatter가 라이선스(apache-2.0)와 데이터셋(bookcorpus, wikipedia)을, `config.json`이 아키텍처를 그대로 담고 있음을 확인했습니다. 외부 도구가 막히면 이 메타데이터를 직접 읽어 modelCard를 구성하는 경량 추출기로 대체할 수 있습니다(난이도 낮음).
...
- 생성 엔진은 미정입니다. cdxgen `aibom`이 CycloneDX 1.7을 내는 점은 맞지만, 생성 스파이크(부록 A)에서 12.5.0까지 HF 모델 ingestion이 없음을 확인했습니다. cdxgen의 모델 ingestion 가능 버전 확인, OWASP AIBOM Generator 평가, 경량 커스텀 추출기 중에서 1차 엔진을 정해야 합니다.
- 자동화 한계(비표준 라이선스 해석, 라이선스 드리프트, 데이터셋 출처 검증)는 문서와 리포트에서 명확히 선을 그어, 도구가 보증을 준다는 인상을 주지 않습니다.

확정된 것은 포맷(AI 경로 1.7)입니다. 남은 미결정은 두 가지입니다. 첫 구현 역량의 범위, 그리고 AIBOM 생성 엔진 선택(부록 A 후속)입니다. 생성은 어느 엔진을 쓰든 출력 회귀 스냅샷으로 specVersion·필드 변화를 지켜봅니다.

> 외부 등록·노출 전략은 [외부 등록 채널](seo-external-listings.md)을, AI SBOM 기능을 더했을 때의 CycloneDX Tool Center 역량 표기 갱신은 같은 문서를 참고하세요.

---

## 부록 A. 생성 스파이크 결과 (2026-06-23)

본격 구현 전에 cdxgen `aibom`의 실제 동작을 시간 제한 스파이크로 확인했습니다. 목적은 두 가지였습니다. 포맷 결정(1.7) 검증과 cdxgen이 HF 모델에서 modelCard를 실제로 만드는지 확인.

**실행 내용**
- cdxgen 12.2.0(로컬)과 핀 버전 12.5.0(npx)으로 `-t aibom` 실행.
- 입력 세 형태: HF 모델 URL(`huggingface.co/prajjwal1/bert-tiny`), 가중치를 제외하고 받은 HF 모델 디렉토리(`bert-base-uncased`, `config.json`+`README.md` frontmatter 포함), 최소 `Modelfile`.
...
- 스파이크가 며칠 비용으로 로드맵의 핵심 가정 하나를 교정했습니다. 적합성 검사 기능을 먼저 크게 만들지 않은 판단이 맞았습니다.
