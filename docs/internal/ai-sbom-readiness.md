...
1. **G7 최소요소 적합성 검사** (구현됨, 이후 전체 클러스터로 확장). `validate-sbom.sh`가 SBOM에 machine-learning-model 컴포넌트가 있으면 G7 검사를 `_conformance.*` 리포트에 덧붙입니다. 초기 6개 모델·데이터셋 체크에서, **선언적 레지스트리(`docker/lib/g7-registry.json`) 기반 평가기**로 바꿔 7개 클러스터 50개 요소 전체를 다룹니다. 레지스트리는 각 요소를 CycloneDX 존재-검사식(cdxPath)과 데이터 출처(auto·직접 읽음 / inferred·신호 도출 / declared·사람·매니페스트 제공 / na·자동 출처 없음)로 기술하고, 평가기는 이를 한 번의 jq 프로그램으로 채점합니다. 모두 권고(warn)라 전체 result를 fail시키지 않습니다. 자동 출처가 없는 시스템 수준·보안 통제·KPI·데이터셋 민감도 요소는 조용히 빼지 않고 "사람 검토 필요(na)"로 표면화해, 리포트가 50개 전체 그림과 도구의 실제 자동 커버 범위를 함께 보이도록 합니다. 개방성은 별도 요소가 아니라 원문에서 Model license의 예시 측면이므로 그 하위 체크로 묶습니다. AIBOM 모드와 ANALYZE(공급사 AI SBOM) 양쪽에서 동작.
...
3. **AIBOM 생성 모드** (구현됨, OWASP 생성기). HuggingFace 모델 id를 OWASP AIBOM Generator로 CycloneDX 1.7 ML-BOM으로 생성하고 기존 후처리에 합류시킵니다(부록 B). 생성 직후 `enrich-aibom.sh`가 자동 채움을 보강합니다.

4. **자동 채움 보강** (구현됨). 생성기가 비워 두지만 G7이 요구하는 필드를, 생성기가 참조하지 않는 출처에서 채웁니다. (a) 모델 무결성 해시 — HuggingFace API가 LFS 가중치 파일마다 SHA-256을 제공하므로(`info.siblings[].lfs.sha256`) 이를 읽어 `hashes[]`에 주입합니다(비-LFS 파일은 git SHA-1뿐이라 대상 아님). (b) 개방성 4축 — gated/private, 가중치 파일, 선언된 데이터셋, 학습 문서화 신호로 도출해 `openness:*` 속성으로 기록하고, 근거로 Model Openness Framework(arXiv 2403.13784)를 인용합니다. (c) 모델 계보·성능 지표 — `cdxgen -t ai`가 채우는 pedigree/performanceMetrics/`cdx:huggingface:*` 속성을 harvest해 병합합니다(cdxgen은 opt-in aibom 이미지에만 설치, 없으면 건너뜀). 모두 best-effort라 네트워크·도구가 없으면 해당 필드를 비운 채 두고, 적합성 검사가 "not present"로 정직하게 보고합니다.

5. **인프라·보안 클러스터 결합** (구현됨). 모델 ML-BOM과 앱 소프트웨어 SBOM을 `merge-sbom.sh`로 합쳐 인프라(소프트웨어 의존성) 클러스터를 채우고, 이어 기존 Trivy 보안 스캔이 그 의존성에 CVE를 붙여 보안 클러스터의 취약점 참조를 채웁니다. 병합은 `MERGE_ROOT_FROM`으로 ML-BOM을 기준 루트로 지정해 specVersion 1.7과 modelCard를 보존합니다. 규제 대응의 2층 구조(일반 SBOM 위에 AI 요소 누적)를 실제로 구현하는 부분입니다.
...
> 갱신(2026-07): 이 "0 components"는 당시 핀 버전(12.2.0/12.5.0)의 한계였고, cdxgen은 v12.5.1(2026-06)에서 `-t ai`로 HuggingFace 모델 수집을 지원하게 됐습니다. 이제 modelCard·라이선스·pedigree(계보)·performanceMetrics·`cdx:huggingface:*` 속성까지 채웁니다. 그래서 1차 생성 엔진은 OWASP 생성기로 유지하되, cdxgen이 추가로 뽑는 pedigree·성능 지표는 `enrich-aibom.sh`가 harvest해 병합하는 방식(6절 4항)으로 재활용합니다.
