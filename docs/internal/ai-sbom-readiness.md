...
- 생성 엔진은 **OWASP AIBOM Generator**를 1차로 권고합니다. 사전 검증(부록 B)에서 HuggingFace 모델을 실제로 읽어 모델 카드·라이선스·데이터셋을 뽑고 CycloneDX 1.6·1.7을 모두 출력함을 확인했습니다. cdxgen `aibom`은 1.7을 내지만 현재 버전이 모델을 못 읽어 제외했습니다. 커스텀 추출기는 대비책으로 둡니다.
